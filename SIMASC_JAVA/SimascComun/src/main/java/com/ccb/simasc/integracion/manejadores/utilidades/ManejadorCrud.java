...
import static com.ccb.simasc.transversal.utilidades.UtilConstantes.NOT_NULL_VALUE;
import static com.ccb.simasc.transversal.utilidades.UtilConstantes.NULL_VALUE;
...
import com.ccb.simasc.integracion.enumeraciones.FuncionAgrupamientoJPQL;
...
import com.ccb.simasc.integracion.enumeraciones.TipoQuery;
import com.ccb.simasc.transversal.utilidades.MensajesEnum;
import com.ccb.simasc.transversal.utilidades.MensajesSimasc;
import com.ccb.simasc.transversal.utilidades.UtilReflection;
...
public abstract class ManejadorCrud<T,U> implements IManejadorCrud<T,U> {
     ...
     */
    @Override
    public T buscar(U pId, LockModeType lockMode) {
        return mp.find(getClaseEntidad(), pId, lockMode);
    }
     ...
     */
    @Override
    public T buscar(U pId, EntityManager em) {
        return mp.find(getClaseEntidad(), pId, em);
    }
    ...
    public List<T> consultar(Collection<InformacionFiltro> filtros, Collection<InformacionOrdenamiento> informacionOrdenamiento, RangoConsulta rangoConsulta) {
        ...
        return (List<T>) mp.listData(q);
    }        
    
    public void openTransaction(){
      	mp.getEntityManager().flush();
    	mp.getEntityManager().getTransaction().begin();
    }
    
    public void closeTransaction(){
    	mp.getEntityManager().getTransaction().commit();
    }
    ...
    public List<Object> consultarLista(Collection<InformacionFiltro> filtros, Collection<InformacionOrdenamiento> infoOrdenamiento, InformacionAgrupamiento infoAgrupamiento){
        ...
        return (List<Object>) mp.listData(q);
    }
     ...
     */
    @Override
    public void crear(T pData, EntityManager em) {
        preGuardar();
        preCrear();
        mp.create(pData, em);
        postCrear();
     ...
     */
    @Override
    public void crearSinAtributosAuditoria(T pData) {
        preGuardar();
        preCrear();
        mp.createSinAtributosAuditoria(pData);
        postCrear();
        postGuardar();
    }
    
    /**
     * Crea la lista de objetos que se pasan como parámetro, seteando
     * los atributos de auditoría.
     * @param lista
     */
    public void crearLista(List<T> lista){
    	for(T objeto : lista){
    		crear(objeto);
    	}
    }
     ...
     */
    @Override
    public void actualizarSinFlush(T pData) {
        preGuardar();
        preActualizar();
        mp.updateWithoutFlush(pData);
        postActualizar();
        postGuardar();
    }
     ...
     */
    @Override
    public void actualizarSinAtributosAuditoria(T pData) {
        preGuardar();
        preActualizar();
        mp.actualizarSinAtributosAuditoria(pData);
        postActualizar();
        postGuardar();
    }
     ...
     */
    @Override
    public void actualizarLista(List<T> lista) {
        for(T obj : lista){
        	this.actualizar(obj);
        }
    }
     ...
     */
    @Override
    public void actualizar(T pData, EntityManager em) {
        preGuardar();
        preActualizar();
        mp.update(pData, em);
        postActualizar();
    ...
    /**
     * Elimina fisicamente el objeto que se pasa como parámetro
     * @param pData
     */
    public void eliminarFisicamente(T pData){
    	mp.delete(pData);
    }
    
    /**
     * Elimina fisicamente la lista de objetos que se pasa como parametro
     * @param lista
     */
    public void eliminarListaFisicamente(List<T> lista){
    	for(T objeto : lista){
    		eliminarFisicamente(objeto);
    	}
    }
    ...
    private Query construirJpqlQuery(Collection<InformacionFiltro> filtros, Collection<InformacionOrdenamiento> informacionOrdenamiento,
        ...
        if(tipoQuery==null){
            throw new RuntimeException(MensajesSimasc.getInstancia().getMensajePorKey(MensajesEnum.ERROR091.toString()));
        }
        ...
        for (InformacionFiltro filtro : filtros) {
            if (filtro.valor != null) {                
                if (jpqlWhere.length() > 7) {
                    if(TipoOperador.AND.equals(filtro.operador)) jpqlWhere.append(" AND ");
                    if(TipoOperador.OR.equals(filtro.operador)) jpqlWhere.append(" OR ");
                }
     ...
     */
    protected String obtenerNombreParametro(String nombreAtributo){
        return nombreAtributo.replaceAll("[.]", "_");
    }
    
    
    public EntityManager getEntityManager(){
    	return mp.getEntityManager();
    }
